...
|---------|----------------|
| **`news-manager ingest`** | Runs the **Supabase-backed ingest pipeline** (load sources from the DB, fetch articles, Groq filter/summarize, write **`news_articles`** / exclusions). |
| **`news-manager user-sources export`** | Prints one user’s **categories + sources** as JSON to **stdout** (looks up user by **email** via Auth admin API; needs **service role**). |
