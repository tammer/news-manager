...
|----------|----------------|
| **`POST /api/sources/resolve`** | Turn a loose **`query`** (URL fragment or site name) into a suggested homepage or RSS URL for ingest—lookup only; does not write to your database. |
...
| **`POST /api/pipeline/run`** | Queue an **async** full **ingest** run (fetch → filter/summarize → Supabase) for the authenticated user, optionally scoped by category/source. |
