...
| **`POST /api/sources/discover`** | Start an **async** source-discovery job from plain-English intent (for example, “independent cybersecurity newsletters”). Returns a `job_id`; use status endpoint to read ranked suggestions with `name`, `url`, and short `why`. |
