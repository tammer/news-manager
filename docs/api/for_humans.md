...
| **`POST /api/sources/discover`** | Start an **async** source-discovery job from plain-English intent. Discovery uses **Groq** to generate several **DuckDuckGo** queries, searches worldwide, rolls hits up by **domain**, then runs a **batched LLM judge** on snippets (no per-page HTML fetch). Results include judged domains with **score ≥ 4**. Each **`url`** is chosen by the judge from real **DuckDuckGo result URLs** for that site (or falls back to `https://<domain>/`), plus `score`, `kind`, `title`, and `reason`. Existing user sources are excluded by URL/domain. Returns `job_id`; use status endpoint for results. |
