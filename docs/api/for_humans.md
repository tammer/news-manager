...
| **`POST /api/sources/discover`** | Start an **async** source-discovery job from plain-English intent. Discovery now runs from DDG query `blogs or news sites about <intent>`, then processes URLs depth-first. Each fetched page is classified by LLM as `irrelevant`, `follow`, or `is_index`. `follow` pages recurse one level into child links; `is_index` pages become suggestions; `irrelevant` pages are skipped. Stops after 5 suggestions (or exhaustion). Returns `job_id`; use status endpoint for results. |
